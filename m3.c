...
int checkout(struct play* player)
{
	...
	printf("Player %c checkedout for $%d score \n", player->playerName, player->score);
	return player->score;
}
void playGame(unsigned int boardSize, struct play players[], unsigned int playerCount)
{
	...
	while (check != 'C')
	{
		...
		for ( i = 0; i < playerCount; i++)
		{
			...
			printf("scroe of player %c is : %d invantory( %d items):", players[i].playerName, players[i].score, players[i].invantory);// print current inventory//
			int n;
			for ( n = 0; n < 10; n++)
			{
				...
				if (players[i].position == players[n].position && n != i)
				{
					...
					if (steal > -1)                    // if the other player has a item //
					{
						...
						for (a = 0; a < 10; a++)
						{
							...
							else if (a == 9)                   // if invatory is full //
								printf("Invantory is full\n");
						}
...
int main(void)
{
	...
	while (select != 'q')
	{
		...
		if (select == 'p')     // play game //
		{
			...
			for ( i = 0; i < playercount; i++)    // get the hight score player ID and score //
			{
				if (player[i].score > highscore)
				{
					highplayer = player[i].playerName;
					highscore = player[i].score;
				}
			}
			printf("Player %c has win the game scroe is %d\n", highplayer, highscore);
		}
